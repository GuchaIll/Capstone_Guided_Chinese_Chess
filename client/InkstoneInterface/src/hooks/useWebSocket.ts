...
// `url` accepts either a static string or an async resolver. The
// resolver pattern is used by the BFF ticket flow: each (re)connect
// fetches a fresh single-use ticket from /api/bridge/ws-ticket and
// returns ws://…/ws?token=<ticket>. Tickets expire 30 s after issue
// so we cannot cache the URL across reconnects.
...
export function useWebSocket({
  ...
  const connect = useCallback(async () => {
    ...
    try {
      resolvedUrl = typeof url === 'string' ? url : await url();
    } catch (error) {
      console.error('[WS Hook] URL resolver failed:', error);
      // Treat resolver failures the same as a closed socket so the
      // reconnect machinery kicks in (e.g. ws-ticket endpoint blip).
      ...
      if (reconnectAttemptsRef.current < maxReconnectAttempts) {
        ...
        reconnectTimeoutRef.current = window.setTimeout(() => {
          if (mountedRef.current && !intentionalCloseRef.current) connect();
        }, delay);
      }
      return;
    }
    ...
    console.log('[WS Hook] Connecting to', resolvedUrl);
