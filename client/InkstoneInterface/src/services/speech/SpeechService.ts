import { dashboardFetch } from '../coachClient';
...
export class SpeechService {
  ...
  private async speakWithFishModal(text: string): Promise<void> {
    // Routes through the BFF (/api/dashboard/tts → go-coach /dashboard/tts);
    // the BFF attaches the real bearer server-side.
    const response = await dashboardFetch('/tts', {
      method: 'POST',
