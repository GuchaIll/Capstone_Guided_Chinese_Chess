...
//
// Both endpoints route through the BFF: bridgeFetch hits
// /api/bridge/kibo/trigger; coachFetch hits /api/coach/classify-move.
// The browser never sees STATE_BRIDGE_TOKEN — see services/bridgeClient
// and services/coachClient for the contract.
//
...
import { coachFetch } from './coachClient';
...
export async function classifyMove(
  ...
  try {
    const response = await coachFetch('/classify-move', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
